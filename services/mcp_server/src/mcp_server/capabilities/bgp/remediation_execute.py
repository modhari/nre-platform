...
def _lattice_mcp_base_url() -> str:
    return os.getenv("BGP_REMEDIATION_PLAN_URL", "http://lattice:8080").rstrip("/")
...
def remediation_execute(request: dict) -> dict:
    ...
    try:
        result = _post_lattice_capability(
            capability_name="bgp_remediation_execute",
            payload=payload,
        ...
        return {
            ...
            "ok": False,
            "error": {
                "code": "bgp_remediation_execute_failed",
                "message": str(exc),
            },
        }
